...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await BooksService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await BooksDBApi.findAll(req.query);
...
router.get(
  '/count',
  wrapAsync(async (req, res) => {
    const payload = await BooksDBApi.findAll(req.query, { countOnly: true });
