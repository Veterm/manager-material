...
import * as dataFormat from 'pages/CRUD/Books/table/BooksDataFormatters';
...
const BooksTable = () => {
  ...
  const count = useSelector((store) => store.books.list.count);
  const modalOpen = useSelector((store) => store.books.list.modalOpen);
  const rows = useSelector((store) => store.books.list.rows);
  const idToDelete = useSelector((store) => store.books.list.idToDelete);
  ...
  const getBooksCSV = async () => {
    const response = await axios({
      url: '/books?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Books')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/books/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/books/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Books'
                : 'http://localhost:8080/api-docs/#/Books'
            }
          >
            API documentation for books
          </LinkMaterial>
        </div>
      </Widget>
